...
- Github: <https://github.com/huabeitech/customer-service-system/releases/tag/${tag}>
- Gitee: <https://gitee.com/huabeitech/customer-service-system/releases/tag/${tag}>
```
...
- GitHub: `GITHUB_TOKEN` or `GH_TOKEN` with access to `huabeitech/customer-service-system` and permission to create releases. For a fine-grained PAT, use an organization-allowed lifetime and grant the repository at least `Contents: Read and write` plus `Metadata: Read`.
- Gitee: `GITEE_ACCESS_TOKEN` or `GITEE_TOKEN` with release write access to `huabeitech/customer-service-system`.
...
```bash
mkdir -p /tmp/customer-service-system-release
awk 'BEGIN{p=0} /^## v1\.2\.3 /{p=1; next} /^## v[0-9]/{if(p) exit} p{print}' \
  docs/zh/docs/changelog.md | sed '/^### 发布地址/,$d' > /tmp/customer-service-system-release/v1.2.3-zh.md
awk 'BEGIN{p=0} /^## v1\.2\.3 /{p=1; next} /^## v[0-9]/{if(p) exit} p{print}' \
  docs/en/docs/changelog.md | sed '/^### Release Links/,$d' > /tmp/customer-service-system-release/v1.2.3-en.md
{
  printf '## 更新内容\n\n'
  sed '1,/^### 更新内容$/d' /tmp/customer-service-system-release/v1.2.3-zh.md
  printf '\n## Updates\n\n'
  sed '1,/^### Updates$/d' /tmp/customer-service-system-release/v1.2.3-en.md
} > /tmp/customer-service-system-release/v1.2.3-release-body.md
```
...
curl -sS -o /tmp/github_release_v1.2.3.json -w '%{http_code}' \
  -X POST https://api.github.com/repos/huabeitech/customer-service-system/releases \
  -H "Authorization: Bearer ${token}" \
  -H 'Accept: application/vnd.github+json' \
  -H 'X-GitHub-Api-Version: 2022-11-28' \
  -H 'Content-Type: application/json' \
  -d @<(jq -n --rawfile body /tmp/customer-service-system-release/v1.2.3-release-body.md \
...
curl -sS -o /tmp/gitee_release_v1.2.3.json -w '%{http_code}' \
  -X POST https://gitee.com/api/v5/repos/huabeitech/customer-service-system/releases \
  -H 'Content-Type: application/json' \
  -d @<(jq -n --rawfile body /tmp/customer-service-system-release/v1.2.3-release-body.md --arg token "${token}" \
...
- Confirm `curl -sS -o /tmp/github_release_verify.json -w '%{http_code}' https://api.github.com/repos/huabeitech/customer-service-system/releases/tags/v1.2.3` returns `200`.
- Confirm `curl -sS -o /tmp/gitee_release_verify.json -w '%{http_code}' https://gitee.com/api/v5/repos/huabeitech/customer-service-system/releases/tags/v1.2.3` returns `200`.
