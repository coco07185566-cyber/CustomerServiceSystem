...
// i18n value that is safe to use during store initialisation (module-level).
// translateCurrentMessage calls readStoredLocale() which returns different
// results on server vs client, causing SSR hydration mismatches. Use this
// instead for initial state so both sides agree.
function tInitial(key: string) {
  return translateMessage(DEFAULT_LOCALE, key)
}
...
export const useSupportChatStore = create<SupportChatStore>((set, get) => {
  ...
  return {
    title: tInitial("supportChat.title"),
    subtitle: "",
    themeColor: "#2563eb",
    conversation: null,
    messages: [],
