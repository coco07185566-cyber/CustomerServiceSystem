...
func InitDB(cfg config.DBConfig) (*gorm.DB, error) {
	...
	// This prevents "database is locked" (SQLITE_BUSY) errors under concurrent access.
	if cfg.Type == "sqlite" {
		if err := db.Exec("PRAGMA journal_mode=WAL").Error; err != nil {
			return nil, fmt.Errorf("enable wal mode: %w", err)
		}
		// Set busy timeout to avoid "database is locked" when a write is in progress.
		// The DSN parameter _busy_timeout may not be supported by the pure-Go driver,
		// so we set it explicitly via PRAGMA as well.
